...
public interface CartLineRepository extends JpaRepository<CartLine, Long> {
    ...
    @Query("SELECT ci.product FROM CartLine ci WHERE ci.product.id = ?1")
    Product findProductById(Long productId);

//	@Query("SELECT ci.cart FROM CartItem ci WHERE ci.product.id = ?1")
//	List<Cart> findCartsByProductId(Long productId);

    @Query("SELECT ci FROM CartLine ci WHERE ci.cart.id = ?1 AND ci.product.id = ?2")
    CartLine findCartItemByProductIdAndCartId(Long cartId, Long productId);
...
//	Order findOrderByEmailAndOrderId(String email, Integer orderId);
    ...
    @Modifying
    @Query("DELETE FROM CartLine ci WHERE ci.cart.id = ?1 AND ci.product.id = ?2")
